function HomePage() {
  return (
    <>
      <div className="site-container">
        <div className="space-y-4">
          <h1 className="text-2xl font-bold">Selamlar, ben Akın.</h1>

          <p>Front-End teknolojileri ile ilgileniyorum. Fmv Işık Üniversitesi Görsel İletişim Tasarımı mezunuyum. </p>

          <p>Uzun zamandır soyut ve somut tasarım çıktılarının olduğu işler yapmaktayım. Serigrafi, Litografi gibi baskı teknikleri ile boş zamanlarımda hobi amaçlı, ihtiyacım olduğu zamanlarda ticari olarak ürünler ortaya çıkartıyorum. Geleneksel tasarımın dışında web, web tipografisi ve kullanıcı arayüzü tasarımı alanlarıyla ilgileniyorum.</p>
          <a href="https://docs.google.com/document/d/1bu3Edfo3PFMoyoxqpBcEFlKnUpsrsd2BectDYir3vgM/edit">CV</p>
        </div>
      </div>
      <div className="site-4xl-container mt-20">
        <img src="/albatros.jpg"></img>
      </div>
    </>
  )
}

export default HomePage
